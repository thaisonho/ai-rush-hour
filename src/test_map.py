...
class ExperimentRunner:
    def __init__(self, map_directory='map', results_directory='experiment_results'):
        # Get the script's directory
        script_dir = Path(__file__).parent
        
        # Set paths relative to script directory
        self.map_directory = script_dir / map_directory
        self.results_directory = script_dir / results_directory
    ...
    def run_experiments(self):
        """Run experiments on all maps"""
        print("Starting experiments...")
        
        # Get all map files
        map_files = sorted([f for f in self.map_directory.glob('*.txt') 
    ...
    def generate_insights(self, df):
        ...
        for line in insights[:15]:  # Print first few insights
            print(line)
        print("\nFull analysis saved to experiment_results/insights.md")
